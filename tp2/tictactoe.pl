...
colonne(C,M) :-
    length(K,M),
    findall(E,(nth1(K,L,E), ligne(L,M)),C),
    K1 is K-1,
    .*/
...
alignement_gagnant([E|L], J) :- 
    ground([E|L]),
    E = J,
    alignement_gagnant(L,J).

alignement_perdant([],_).
alignement_perdant([E|L], J) :- 
    ground([E|L]),
    adversaire(J,A),
    E = A,
    alignement_perdant(L,J).
...
heuristique(J,Situation,H) :- 
    findall(Ali,(alignement(Ali,Situation),possible(Ali,J)),AliP),
    length(AliP,P1),
    adversaire(J,Adv),
    findall(Ali,(alignement(Ali,Situation),possible(Ali,Adv)),AliPAdv),
    length(AliPAdv,P2),
    H is P1-P2.
